 ...
 */
 ...
 void freeType(Type* type) {
   ...
   case TP_ARRAY:
     ...
     free(type); // Sửa lỗi rò rỉ bộ nhớ: free(type) thay vì freeType(type)
     break;
   }
 }
 ...
 Object* lookupObject(char *name) {
   // Bắt đầu tìm kiếm từ scope hiện tại
   ...
   while (currentScope != NULL) {
     ...
     // Nhảy lên scope ngoài (outer scope)
     currentScope = currentScope->outer;
   }
 
   // Tìm kiếm trong danh sách đối tượng toàn cục
